// server/services/interventionGenerator.js (CODICE CORRETTO DEL SERVIZIO)

const mongoose = require('mongoose');
// CORRETTO: Importa i modelli dal file models/diagnosi.js e models/progettazione.js
const { Checklist, Gap } = require('../models/diagnosi');
const { Intervento } = require('../models/progettazione'); // <-- Importa il modello Intervento

// --- IMPORTA LE UTILITY DAL NUOVO FILE E actionPlanRules ---
const { mapArea, mapPriorita, getTiming } = require('../utils/mappingHelpers'); // Adatta il path se necessario
const { actionPlanRules } = require('../knowledge/actionPlanRules'); // Solo le regole ora
// ---------------------------------------------------------
...
const path = require('path'); // Aggiunto per dotenv
// --------------------

dotenv.config({ path: path.join(__dirname, '../../.env') }); // Adatta il path se necessario

const openaiApiKey = process.env.OPENAI_API_KEY;
// Potresti usare un modello diverso per la generazione di interventi
const MODEL_FOR_INTERVENTION_ENRICHMENT = process.env.OPENAI_MODEL_FOR_INTERVENTION || "gpt-4o-mini";
const openai = openaiApiKey ? new OpenAI({ apiKey: openaiApiKey }) : null;

/**
 * Arricchisce un suggerimento di intervento base con dettagli e riferimenti KB tramite AI.
 * @param {object} gap Il documento Gap (arricchito con info AI) che ha triggerato l'intervento.
 * @param {object} rule La regola base da actionPlanRules.js per l'intervento.
 * @param {object} clienteInfo Informazioni sul cliente.
 * @returns {Promise<object|null>} Oggetto con i dati arricchiti per l'intervento o null.
 */
async function enrichInterventionSuggestionWithAI(gap, rule, clienteInfo) {
    ...
    try {
        ...
        if (!responseContent) { throw new Error("Risposta AI vuota per arricchimento intervento."); }
        ...
        return null; // Fallback o gestione errore
    }
}

/**
 * Genera automaticamente documenti Intervento nel DB basandosi sui Gap di una Checklist.
 * Ora gli interventi sono arricchiti dall'AI.
 */
async function generateInterventionsFromGaps(checklistId) {
    ...
    const relevantGaps = await Gap.find({ // Recuperiamo i GAP GIA' ARRICCHITI
        checklist_id: checklistId,
        livello_rischio: { $in: riskLevelsToInclude },
        arricchitoConAI: true // Considera solo i gap che hanno completato l'arricchimento precedente
    }).lean();
    ...
    for (const gap of relevantGaps) {
        ...
        if (rule) {
            // Chiamata all'AI per arricchire il suggerimento di intervento
            ...
            if (enrichedInterventionData) {
                const newInterventoData = {
                    ...
                    area: mapArea(gap.item_id), // Chiamata alla funzione importata
                    priorita: mapPriorita(rule.prioritaDefault), // Chiamata alla funzione importata
                    gap_correlati: [gap._id],
                    stato: 'suggerito', // Stato iniziale per interventi suggeriti dall'AI
                    origin: 'ai_generated',
                    checklist_id_origine: checklistId,
                    ...
                    tempistica_stimata: enrichedInterventionData.tempistica_stimata, // Già calcolata da enrichIntervention...
                    risorse_necessarie: enrichedInterventionData.risorse_necessarie, // Già presa da rule in enrichIntervention...
                };
        ...
        await new Promise(resolve => setTimeout(resolve, 100)); // Piccolo delay per evitare rate limiting
    }
...
// Esporta solo la funzione del servizio
module.exports = { generateInterventionsFromGaps };