// server/services/summaryGeneratorAI.js
...
// Scegli un modello capace di sintesi e ragionamento strategico
const modelToUse = process.env.OPENAI_MODEL_FOR_SUMMARY || "gpt-4.1-nano"; 
const openai = openaiApiKey ? new OpenAI({ apiKey: openaiApiKey }) : null;

/**
 * Genera una bozza di Executive Summary per il report diagnostico usando l'AI.
 * @param {object} reportDataConsolidato - Dati del report già parzialmente elaborati da reportGenerator.js
 *                                         (include profilo cliente, statistiche gap, giudizi base, aree forza/debolezza base).
 * @param {object} clienteInfo - Oggetto cliente con obiettiviStrategici e criticitaPercepite.
 * @returns {Promise<string>} La bozza dell'executive summary.
 */
const generateExecutiveSummaryAI = async (reportDataConsolidato, clienteInfo) => {
    ...
    // Fornire un elenco più dettagliato dei gap principali (i primi 5-7 ad alto/medio rischio)
    if (reportDataConsolidato.elencoGapCompleto && reportDataConsolidato.elencoGapCompleto.length > 0) {
        ...
        reportDataConsolidato.elencoGapCompleto
            .filter(g => g.livello_rischio === 'alto' || g.livello_rischio === 'medio')
            .slice(0, 7) // Limita ai primi 7 per brevità nel prompt
            .forEach(g => {
                userPrompt += `- ID: ${g.item_id}, Rischio: ${g.livello_rischio}, Descrizione: ${g.descrizione}\\n`;
                // Aggiungi le implicazioni arricchite dall'AI (D3) se disponibili
    ...
    try {
        const completion = await openai.chat.completions.create({
            ...
            temperature: 0.4, // Un po' di creatività per la sintesi strategica
            // max_tokens: 600, // Adeguato per 400 parole
        });
...
module.exports = { generateExecutiveSummaryAI }; 