...
const ReportPage = () => {
    ...
    useEffect(() => {
        ...
        const fetchOrGenerateReport = async (force = false) => {
            if (!selectedChecklistId) {
                setReportData(null); setErrorReport(null); setReportDocumentId(null);
                return;
            }
            setLoadingReport(true); setErrorReport(null); setReportData(null); setReportDocumentId(null);
            try {
                const url = `http://localhost:5001/api/report?checklist_id=${selectedChecklistId}${force ? '&force_generate=true' : ''}`;
                console.log(`>>> ReportPage: Fetching/Generating report from ${url}`);
                const response = await axios.get(url);
                
                if (response.data && response.data.data) {
                    setReportData(response.data.data);
                    setReportDocumentId(response.data.data._id);
                    console.log(`>>> ReportPage: Report data (ID: ${response.data.data._id}) received:`, response.data.data);
                    if (force) {
            ...
            const fetchForced = async () => {
                ...
                try {
                    ...
                    if (response.data && response.data.data) {
                        ...
                        setReportDocumentId(response.data.data._id);
                        setGenerationMessage({ type: 'success', text: 'Report rigenerato con successo!' });
                    } else { throw new Error("Dati del report non validi."); }
                } catch (err) { 
    ...
    const handleExportPDF = async () => {
        ...
        setErrorReport(null);
        setGenerationMessage({ type: '', text: '' });
        console.log(`Inizio richiesta export PDF per report: ${reportDocumentId}`);
                    ...
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 2 }}>
                        <Box sx={{ flexGrow: 1 }}>
                            ...
                            <Typography variant="caption" display="block" color="text.secondary">
                                Stato Checklist: {reportData.checklistInfo?.stato} 
                                {reportData.checklistInfo?.percentuale_completamento !== undefined && ` (${reportData.checklistInfo.percentuale_completamento}%)`}
                            </Typography>
                        ...
                        <Stack direction="row" spacing={1} sx={{ mt: { xs: 2, md: 0 } }}>
                            <Tooltip title="Forza la rigenerazione completa dei dati di questo report, incluse le analisi AI. Potrebbe richiedere tempo.">
                                <span>
                                    <Button
                                        variant="outlined"
                                        size="small"
                            ...
                            <Tooltip title="Esporta il report attualmente visualizzato in formato PDF.">
                                <span>
                                    <Button 
